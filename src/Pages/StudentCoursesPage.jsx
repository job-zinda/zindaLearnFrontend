...
function CourseFallback({ seedValue, categoryKey, sectionType }) {
  return (
    <div className={`student-course-fallback student-course-fallback--${getFallbackVariant(seedValue)}`}>
      <span>{getFallbackIcon(categoryKey, sectionType)}</span>
    </div>
  );
}

function CourseCard({ course, categoryKey, onViewTutors }) {
  return (
    <article className="student-course-card">
      <div className="student-course-card__image">
        {course.image ? (
...
export default function StudentCoursesPage() {
  ...
  function openCourseTutors(course) {
    navigate(`/student/courses/${categoryId}/tutors/${course._id}`, {
      state: {
  ...
  if (loading) {
    return (
      <div className="student-courses-page">
        <div className="student-course-state">Loading courses...</div>
      </div>
    );
  }
  ...
  return (
    <div className="student-courses-page">
      <div className="student-courses-breadcrumb">
        <button type="button" onClick={() => navigate("/student")}>←</button>
        <button type="button" onClick={() => navigate("/student")}>Home</button>
        <span>»</span>
        <b>Courses</b>
      </div>
      ...
      {categoryKey === "online_tuition" && (
        <div className="student-courses-tabs">
          <button
            type="button"
            className={`student-courses-tab ${
              activeSection === "one_to_one" ? "student-courses-tab--active" : ""
            }`}
            onClick={() => setActiveSection("one_to_one")}
          >
            <span>One-to-One Session</span>
            <small>Get personalized attention with one to one sessions tailored to your learning speed and goals.</small>
          </button>

          <button
            type="button"
            className={`student-courses-tab ${
              activeSection === "batch" ? "student-courses-tab--active" : ""
            }`}
            onClick={() => setActiveSection("batch")}
          >
            <span>Group / Batch Session</span>
            <small>Learn together with peers in structured batch environments improve collaboration and knowledge sharing.</small>
          </button>
        </div>
      )}
      ...
      {displayedCourses.length === 0 ? (
        <div className="student-course-state">No courses found.</div>
      ) : (
